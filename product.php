...
$reviewSent = ($_GET['review'] ?? '') === 'success';
$reviewTotal = count($approvedReviews);
$reviewAverage = $reviewTotal ? array_sum(array_map(fn($review) => (int)$review['rating'], $approvedReviews)) / $reviewTotal : null;
$reviewAverageLabel = $reviewAverage === null ? '—' : number_format($reviewAverage, 1, ',', '.');
$reviewStars = $reviewAverage === null ? '☆☆☆☆☆' : str_repeat('★', (int)round($reviewAverage)).str_repeat('☆', 5 - (int)round($reviewAverage));
$reviewsPerPage = 5;
$reviewPages = max(1, (int)ceil($reviewTotal / $reviewsPerPage));
$reviewPage = min($reviewPages, max(1, (int)($_GET['review_page'] ?? 1)));
$visibleReviews = array_slice($approvedReviews, ($reviewPage - 1) * $reviewsPerPage, $reviewsPerPage);
...
<main>
  ...
  <section class="container pdp">
    ...
    <div class="product-info">
      ...
      <div class="stars"><?= $reviewStars ?> <span style="color:#6c665c;letter-spacing:0"> <?= $reviewAverage === null ? 'Chưa có đánh giá' : $reviewAverageLabel.' · '.$reviewTotal.' đánh giá đã duyệt' ?></span></div>
  ...
  <section class="product-reviews"><div class="container"><div class="section-head"><div><div class="eyebrow">Khách hàng đánh giá</div><h2>Những phòng khách đã chọn <?= htmlspecialchars($p['name']) ?></h2></div><div><div class="review-score"><b><?= $reviewAverageLabel ?>/5</b><span><?= $reviewStars ?></span><small><?= $reviewTotal ?> đánh giá đã duyệt</small></div><button class="under review-open" type="button">Viết review</button></div></div><div class="review-list"><?php foreach ($visibleReviews as $review): ?><article class="customer-review"><div class="review-author"><b><?= htmlspecialchars($review['customer_name']) ?></b><span class="stars"><?= str_repeat('★', (int)$review['rating']) ?></span></div><p><?= nl2br(htmlspecialchars($review['content'])) ?></p><?php if ($review['images']): ?><div class="review-images"><?php foreach ($review['images'] as $image): ?><a data-fslightbox="review-<?= (int)$review['id'] ?>" href="<?= htmlspecialchars($image) ?>"><img src="<?= htmlspecialchars($image) ?>" alt="Ảnh khách hàng"></a><?php endforeach; ?></div><?php endif; ?></article><?php endforeach; ?><?php if (!$approvedReviews): ?><p class="review-empty">Chưa có đánh giá được duyệt. Hãy là người đầu tiên chia sẻ trải nghiệm.</p><?php endif; ?></div><?php if ($reviewPages > 1): ?><nav class="review-pagination" aria-label="Trang đánh giá"><?php if ($reviewPage > 1): ?><a href="?slug=<?= urlencode($p['slug']) ?>&review_page=<?= $reviewPage - 1 ?>#reviews" aria-label="Trang đánh giá trước"><i class="bi bi-arrow-left"></i></a><?php endif; ?><?php for ($i=1; $i<=$reviewPages; $i++): ?><a class="<?= $i === $reviewPage ? 'active' : '' ?>" href="?slug=<?= urlencode($p['slug']) ?>&review_page=<?= $i ?>#reviews"><?= $i ?></a><?php endfor; ?><?php if ($reviewPage < $reviewPages): ?><a href="?slug=<?= urlencode($p['slug']) ?>&review_page=<?= $reviewPage + 1 ?>#reviews" aria-label="Trang đánh giá tiếp"><i class="bi bi-arrow-right"></i></a><?php endif; ?></nav><?php endif; ?></div></section>
  ...
  <dialog class="review-dialog"><form method="post" action="review-submit.php" enctype="multipart/form-data"><button class="review-close" type="button">×</button><div class="eyebrow">Chia sẻ trải nghiệm</div><h2>Viết review</h2><input type="hidden" name="product_id" value="<?= (int)$p['id'] ?>"><label>Tên của bạn *<input required name="customer_name"></label><label>Đánh giá *<select name="rating"><option value="5">5 sao</option><option value="4">4 sao</option><option value="3">3 sao</option><option value="2">2 sao</option><option value="1">1 sao</option></select></label><label>Nội dung review *<textarea required name="content" rows="5"></textarea></label><label>Ảnh thực tế <input type="file" name="images[]" accept="image/jpeg,image/png,image/webp" multiple></label><p>Review sẽ hiển thị sau khi được IMA PRIME duyệt.</p><button class="button">Gửi review</button></form></dialog>
  <dialog class="review-success-dialog" <?= $reviewSent ? 'data-review-success' : '' ?>><button class="review-close" type="button">×</button><i class="bi bi-check2-circle"></i><div class="eyebrow">Đã gửi đánh giá</div><h2>Cảm ơn bạn đã chia sẻ</h2><p>Đánh giá của bạn đã được ghi nhận và sẽ hiển thị sau khi IMA PRIME duyệt.</p><button class="button review-close" type="button">Đã hiểu</button></dialog>
