...
document.querySelectorAll('.media-thumb img').forEach(image=>image.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const thumb=image.closest('.media-thumb'),url=thumb?.querySelector('input[name="featured_image"]')?.value,productId=new URLSearchParams(location.search).get('id');if(!url||!productId)return;const menu=document.createElement('div');menu.className='image-action-modal';menu.innerHTML='<div class="image-action-backdrop"></div><section><img src="'+image.src+'" alt=""><h3>Quản lý ảnh</h3><button data-image-action="primary">Đặt làm ảnh chính</button><button data-image-action="secondary">Đặt làm ảnh phụ</button><button class="danger" data-image-action="delete">Xoá ảnh</button><button class="cancel-image-action">Huỷ</button></section>';document.body.append(menu);const close=()=>menu.remove();menu.querySelectorAll('.image-action-backdrop,.cancel-image-action').forEach(button=>button.addEventListener('click',close));menu.querySelectorAll('[data-image-action]').forEach(button=>button.addEventListener('click',async()=>{const action=button.dataset.imageAction;if(action==='delete'&&!confirm('Xoá ảnh này khỏi sản phẩm?'))return;await fetch('product-image-action.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({product_id:productId,image_url:url,action})});location.reload();}));}));
