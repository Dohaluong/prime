...
<div class="catalog-grid" data-catalog-grid><?php foreach($all as $p): ?><a class="catalog-card" data-catalog-type="<?=htmlspecialchars($p['type'])?>" data-catalog-price="<?=$p['price']?>" data-catalog-fast="<?=$p['fast']?>" data-catalog-rating="<?=$p['rating']?>" href="product.php?slug=<?=urlencode($p['slug'])?>"><div class="catalog-image"><img class="catalog-primary-image" src="<?=htmlspecialchars($p['image'])?>" alt="<?=htmlspecialchars($p['name'])?>"><?php if(!empty($p['image_secondary'])): ?><img class="catalog-secondary-image" src="<?=htmlspecialchars($p['image_secondary'])?>" alt="<?=htmlspecialchars($p['name'])?>"><?php endif; ?><span class="badge <?=$p['fast']?'':'order'?>"><?=$p['fast']?'Có sẵn giao ngay':'Đặt hàng 7–10 ngày'?></span></div><div><div class="catalog-title"><h3><?=htmlspecialchars($p['name'])?></h3><small>★ <?=htmlspecialchars($p['rating'])?></small></div><p><?=htmlspecialchars($p['description'])?></p><b><?=money($p['price'])?></b></div></a><?php endforeach; ?></div>
