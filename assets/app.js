...
document.querySelectorAll('.catalog-layout').forEach(layout=>{const filter=layout.querySelector('.catalog-filter'),open=document.querySelector('[data-catalog-filter-open]'),apply=filter?.querySelector('[data-catalog-filter-apply]'),badge=document.querySelector('[data-catalog-mobile-count]');const sync=()=>{if(!badge||!filter)return;let count=(filter.querySelector('[data-catalog-filter].active')?.dataset.catalogFilter!=='all'?1:0)+(filter.querySelector('[data-catalog-price].active')?.dataset.catalogPrice!=='all'?1:0)+(filter.querySelector('[data-catalog-stock]')?.checked?1:0);badge.textContent=count||'';};open?.addEventListener('click',()=>filter?.classList.add('is-open'));apply?.addEventListener('click',()=>filter?.classList.remove('is-open'));filter?.addEventListener('click',()=>setTimeout(sync));filter?.addEventListener('change',sync);sync();});
